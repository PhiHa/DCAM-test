...
DCAM Device Provisioning Web Portal Design Approved 1.0, DCAM Device Provisioning Web Portal App Design Approved 1.1, DCAM Web Portal & Device API Contract Draft 0.6, DCAM Documentation Governance, DCAM Security & Encryption Design Draft 1.2, DCAM Factory Provisioning & Device Production SOP Draft 1.2
...
Cloud Function có thể dùng HTTPS hoặc callable mapping, nhưng semantics theo API Contract Draft 0.6.

### 5.11 Result / Error Module

Success hiển thị Created/Restored, `dcam_cloud_device_id`, serial, owner, manufacture date, device state và safe next step.

Error phải có safe title/description, reason code nếu phù hợp và action hợp lệ: retry, rescan, login hoặc support required.

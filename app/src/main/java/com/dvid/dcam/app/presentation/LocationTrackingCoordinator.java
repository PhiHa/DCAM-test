...
public final class LocationTrackingCoordinator {
    ...
    public synchronized LocationTrackingState refresh() {
        ...
        coordinate = null;
        state = tracking.start(this::acceptCoordinate, this::acceptState);
        onStateChanged.run();
        return state;
    }

    public synchronized void stop() {
        stopAs(LocationTrackingState.STOPPED);
    }
    ...
    private synchronized LocationTrackingState stopAs(LocationTrackingState stoppedState) {
        tracking.stop();
        coordinate = null;
        state = stoppedState;
        onStateChanged.run();
    ...
    private synchronized void acceptCoordinate(GpsCoordinate value) {
    ...
    private synchronized void acceptState(LocationTrackingState value) {
        if (value == null) return;
        state = value;

...
public final class RefreshRemoteConfigUseCaseImpl implements RefreshRemoteConfigUseCase {
    private final RemoteConfigGateway provider;
    private final RemoteConfigStore store;

    public RefreshRemoteConfigUseCaseImpl(RemoteConfigGateway provider, RemoteConfigStore store) {
        this.provider = provider;
        this.store = store;
    }

    @Override public RemoteConfigSnapshot execute(DeviceCloudIdentity identity) {
